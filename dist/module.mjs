  ...
  async setup(opts, nuxt) {
    ...
    async function generateGqlTypes(hmrDoc) {
      ...
      for await (const path of documentPaths) {
        const files = (await resolveFiles(path, [gqlMatch, "!**/schemas"], { followSymbolicLinks: false })).filter(allowDocument).map((file) => normalize(file));
      ...
      addTemplate({
        ...
        getContents: () => {
          if (!ctx.codegen || !ctx.fns || !ctx.clients) {
            const fnTypes2 = ctx.fns?.map((fn) => `  export const ${config.functionPrefix + upperFirst(fn)}: (...params: any[]) => Promise<any>`).join("\n") || "";
            return [
    ...
    nuxt.hook("vite:extendConfig", (config2, { isServer }) => {
      config2.optimizeDeps?.include?.push("nuxt-graphql-client > graphql-request");
